...
import { GuildUser } from "../../utils/discord/User";
...
export default function GuildCards({ guild, bot } : GuildCardsProps) {
    const isBotInServer = useMemo(() => bot.guilds.find((g) => g.id == guild.id), [guild])
    ...
    return(
        <div ref={ref} className={`min-w-[125px] max-w-[150px] aspect-square bg-zinc-800 rounded-md transition-all duration-200 hover:-translate-y-3 hover:shadow-xl cursor-pointer relative ${inView ? "scale-100": "scale-50"}`} onClick={() => {
            if(isBotInServer) {
                ...
                <div className="w-[48px] h-[48px] rounded-full border-4 border-zinc-800 absolute translate-x-2 translate-y-[-50%] overflow-hidden">
                    { guild.icon ? (
                        ...
                        <div className="w-full h-full bg-neutral-900 text-center">
                            <h1 className={`text-3xl ${isBotInServer ? "text-gray-600" : "text-gray-400"} font-black`}>{guild.name.charAt(0).toLocaleUpperCase()}</h1>
                        </div>
                    )}
                </div>
