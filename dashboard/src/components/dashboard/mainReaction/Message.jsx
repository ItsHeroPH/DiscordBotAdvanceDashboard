...
export default function Message({ config, setConfig }) {
    const isRequire = useMemo(() => !(config.messageEmbed) && config.message == "",[config])
    return (
        <div className="my-3 relative">
            <h1 className="text-md text-gray-500 font-medium">Message</h1>
            <div className="w-full rounded-lg bg-neutral-950">
                <textarea className="w-full min-h-[100px] max-h-[400px] bg-transparent outline-none text-lg text-gray-300 px-3 py-1 font-semibold" required={isRequire} onChange={(e) => setConfig({...config, message: e.target.value })} defaultValue={config.message} placeholder="Message"/>
            </div>
