        ...
        <div className={`w-full sm:w-[400px] p-3 rounded-lg bg-stone-700 border-l-8 flex flex-row flex-wrap justify-between ${config.messageEmbed ? "" : "hidden"}`} style={{borderLeftColor: config.embed.color}}>
            <div className="w-full">
                ...
                <div className="flex flex-col">
                    <div className={`absolute translate-y-[24%] ${embedColorPicker ? "": "hidden"}`}>
                        <ChromePicker className="!bg-neutral-800" color={config.embed.color} onChange={(color) => setConfig({...config, 
                            embed: {...config.embed, color: color.hex }})
                        }/>
                    </div>
                    <div className="relative w-[45px] h-[45px] rounded-lg border-4 border-stone-800 text-end px-1 cursor-pointer" onClick={() => setEmbedColorPicker((prev) => !prev)} style={
                            {
                                backgroundColor: `${config.embed.color}`
                            }
                    }>
                ...
                <div className="w-full sm:w-[300px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                    <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="image url" defaultValue={config.embed.thumbnail} onChange={(e) => 
                        setConfig({...config, embed: {...config.embed, thumbnail: e.target.value }}) 
                    }/>
                </div>
            </div>
            <div className="w-full">
                ...
                <div className="flex flex-row flex-wrap gap-1">
                    <div className="w-full sm:w-[100px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="icon_url" defaultValue={config.embed.author.icon} onChange={(e) => 
                    ...
                    <div className="w-full sm:w-[200px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="name" defaultValue={config.embed.author.name} onChange={(e) => 
                    ...
                    <div className="w-full sm:w-[60px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="url" defaultValue={config.embed.author.url} onChange={(e) => 
                            setConfig({...config, anouncement: {...config, embed: {...config.embed, author: {...config.embed.author, url: e.target.value}}}}) 
                            }/>
                    </div>
                </div>
                ...
                <div className="flex flex-row flex-wrap gap-1">
                    <div className="w-full sm:w-[100px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="url" defaultValue={config.embed.url} onChange={(e) => 
                    ...
                    <div className="w-full sm:w-[250px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="title" defaultValue={config.embed.title} onChange={(e) => 
                ...
                <div className={`w-full sm:w-[350px] h-[35px] my-1 bg-zinc-800 text-lg text-white font-medium text-center py-1 rounded-lg cursor-pointer ${config.embed.fields.length == 25 ? "hidden" : ""}`} onClick={() => 
                ...
                <div className="w-full sm:w-[350px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                    <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="image url" defaultValue={config.embed.image} onChange={(e) => 
                        setConfig({...config, embed: {...config.embed, image: e.target.value} }) }/>
                </div>
                <h1 className="text-md text-gray-300 font-medium">Footer</h1>
                <div className="flex flex-row flex-wrap gap-1">
                    <div className="w-full sm:w-[100px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="icon url" defaultValue={config.embed.footer.icon} onChange={(e) => 
                            setConfig({...config, embed: {...config.embed, url: e.target.value} }) 
                            }/>
                    </div>
                    <div className="w-full sm:w-[250px] h-[35px] rounded-lg border-sky-500 hover:border-2 bg-neutral-800">
                        <input className="w-full h-full bg-transparent outline-none text-sm text-gray-300 px-3 py-1 font-semibold" placeholder="footer" defaultValue={config.embed.footer.text} onChange={(e) => 
                            setConfig({...config, embed: {...config.embed, title: e.target.value} }) 
                            }/>
                    </div>
                </div>
                <div className="flex gap-2 my-1 items-center">
                    <label className={`w-[40px] h-[20px] ${config.embed.timestamp ? "bg-sky-500" : "bg-gray-600"} transition-all duration-200 ease-out transform inline-block relative rounded-full shadow-inner cursor-pointer`} onClick={() => 
                        setConfig({...config, embed: {...config.embed, timestamp: !config.embed.timestamp } })
                    }>
                        <span className={`w-[15px] h-[15px] rounded-full bg-white absolute transition-all duration-200 ease-out transform ${config.embed.timestamp ? "translate-x-[140%]" : "translate-x-[20%]"} top-0.5 bottom-0.5`}/>
                    </label>
        ...
        <div className={`w-full sm:w-[350px] p-4 rounded-lg bg-slate-800/25 hover:bg-slate-400/25 my-1 transition-all ease-in duration-300 ${view ? "scale-100 opacity-100" : "scale-50 opacity-0"}`} ref={ref}>
            <div className="flex justify-between">
                ...
                <div className="text-red-800 text-xl cursor-pointer" onClick={() => 
                    setConfig({...config, embed: {...config.embed, fields: fields.filter((_,i) => i !== index)} })
