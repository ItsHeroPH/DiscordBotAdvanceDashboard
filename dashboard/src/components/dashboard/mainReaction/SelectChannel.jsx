...
export default function SelectChannel({ config, setConfig, channels }) {
    ...
    return (
        <div className="my-3">
            ...
            <div className="flex flex-col relative w-full mb-3 z-30">
                <div className="w-full h-[40px] bg-stone-950 rounded-lg cursor-pointer px-4 flex justify-between items-center" onClick={() => setDropMenu((prev) => !prev)}>
                    <div className="text-white font-medium flex flex-row items-center">
                        ...
                        <input className="bg-transparent outline-none w-full h-fit text-gray-300" required={true} value={channels.find((ch) => ch.id == config.channel)? channels.find((ch) => ch.id == config.channel).name : ""} placeholder="Please Select A Channel"></input>
                    </div>
